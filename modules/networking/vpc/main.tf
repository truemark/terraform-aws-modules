...
locals {
  ...
  #netnum override
  private_netnum_override = {
    standard = local.private_subnets[var.subnet_cidr]
    override = var.private_netnum
  }
  ...
  secondary_cidr_subnet                 = [
    for cidr in var.secondary_cidr : "${cidr.network}${cidr.cidr}"
  ]
  secondary_private_subnets                 = [
    for cidr in var.secondary_private_subnets : "${cidr.network}${cidr.cidr}"
  ]
  secondary_public_subnets                 = [
    for cidr in var.secondary_public_subnets : "${cidr.network}${cidr.cidr}"
  ]
  secondary_intra_subnets                 = [
    for cidr in var.secondary_intra_subnets : "${cidr.network}${cidr.cidr}"
  ]
  secondary_database_subnets                 = [
    for cidr in var.secondary_database_subnets : "${cidr.network}${cidr.cidr}"
  ]
  secondary_elasticache_subnets                 = [
    for cidr in var.secondary_elasticache_subnets : "${cidr.network}${cidr.cidr}"
  ]
  secondary_redshift_subnets                 = [
    for cidr in var.secondary_redshift_subnets : "${cidr.network}${cidr.cidr}"
  ]
  #   endpoints
...
module "vpc" {
  ...
  secondary_cidr_blocks = local.secondary_cidr_subnet
  private_subnets                                                   = concat([for num in range(local.privateno[var.private], length(slice(data.aws_availability_zones.available.names, 0, var.az_count))) : cidrsubnet(local.cidr_subnet, local.private_network_override[var.network_override], num + local.private_netnum_override[var.network_override])],local.secondary_private_subnets)
  public_subnets                                                    = concat([for num in range(local.publicno[var.public], length(slice(data.aws_availability_zones.available.names, 0, var.az_count))) : cidrsubnet(local.cidr_subnet, local.public_network_override[var.network_override], num + local.public_netnum_override[var.network_override])], local.secondary_public_subnets)
  intra_subnets                                                     = concat([for num in range(local.intrano[var.intra], length(slice(data.aws_availability_zones.available.names, 0, var.az_count))) : cidrsubnet(local.cidr_subnet, local.intra_network_override[var.network_override], num + local.intra_netnum_override[var.network_override])], local.secondary_intra_subnets)
  database_subnets                                                  = concat([for num in range(local.databaseno[var.database], length(slice(data.aws_availability_zones.available.names, 0, var.az_count))) : cidrsubnet(local.cidr_subnet, local.database_network_override[var.network_override], num + local.database_netnum_override[var.network_override])], local.secondary_database_subnets)
  elasticache_subnets                                               = concat([for num in range(local.elasticacheno[var.elasticache], length(slice(data.aws_availability_zones.available.names, 0, var.az_count))) : cidrsubnet(local.cidr_subnet, local.elasticache_network_override[var.network_override], num + local.elasticache_netnum_override[var.network_override])], local.secondary_elasticache_subnets)
  redshift_subnets                                                  = concat([for num in range(local.redshiftno[var.redshift], length(slice(data.aws_availability_zones.available.names, 0, var.az_count))) : cidrsubnet(local.cidr_subnet, local.redshift_network_override[var.network_override], num + local.redshift_netnum_override[var.network_override])], local.secondary_redshift_subnets)
